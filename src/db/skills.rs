...
const CACHE_FILE: &str = "db_api_skills.json";
const DB_NAME: &str = "skills";
...
pub static DB: Lazy<Mutex<GenericDatabase<Skill>>> =
...
pub fn ensure_loaded() {
    ...
    std::thread::spawn(move || {
        let result = std::panic::catch_unwind(|| {
            if let Some(entries) = super::load_from_cache::<Skill>(CACHE_FILE) {
                if !entries.is_empty() {
                    log_debug(&format!(
...
fn fetch_new_from_api() {
    ...
    let result = rt.block_on(async {
        ...
        let all_ids = super::fetcher::fetch_all_ids(&client, "/skills").await?;
        ...
        let batch = super::fetcher::batch_fetch::<Skill>(
            &client,
            "/skills",
            &new_ids,
            &|fetched, total| {
        ...
        Ok::<Vec<Skill>, String>(batch.entries)
    });
...
fn fetch_from_api() {
    ...
    let result = rt.block_on(async {
        let client = super::fetcher::make_client()?;
        let ids = super::fetcher::fetch_all_ids(&client, "/skills").await?;
        ...
        let batch =
            super::fetcher::batch_fetch::<Skill>(&client, "/skills", &ids, &|fetched, total| {
                let mut db = DB.lock();
                db.progress = Some((fetched, total));
            })
            .await;
