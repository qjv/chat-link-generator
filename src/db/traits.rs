...
const CACHE_FILE: &str = "db_api_traits.json";
...
fn fetch_new_from_api() {
    ...
    let result = rt.block_on(async {
        ...
        let batch = super::fetcher::batch_fetch::<GwTrait>(
            &client,
            "/traits",
            &new_ids,
            &|fetched, total| {
        ...
        Ok::<Vec<GwTrait>, String>(batch.entries)
    });
...
fn fetch_from_api() {
    ...
    let result = rt.block_on(async {
        ...
        let batch =
            super::fetcher::batch_fetch::<GwTrait>(&client, "/traits", &ids, &|fetched, total| {
                let mut db = DB.lock();
                db.progress = Some((fetched, total));
            })
            .await;
