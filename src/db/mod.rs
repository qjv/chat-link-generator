...
pub fn load_from_cache<T: serde::de::DeserializeOwned>(filename: &str) -> Option<Vec<T>> {
    ...
    log_debug(&format!("[cache] Parsing {} ({} bytes)...", filename, data.len()));
...
pub fn save_to_cache<T: serde::Serialize>(filename: &str, entries: &[T]) {
    log_debug(&format!("[cache] Saving {} ({} entries)...", filename, entries.len()));
    if let Some(path) = get_cache_path(filename) {
        ...
        match serde_json::to_string(entries) {
            Ok(json) => {
                log_debug(&format!("[cache] Serialized {} ({} bytes), writing...", filename, json.len()));
...
pub fn ensure_loaded(link_type: LinkType) {
    log_debug(&format!("[db] ensure_loaded called for {}", link_type.name()));
    match link_type {
...
pub fn get_status(link_type: LinkType) -> (DbStatus, usize, String, Option<(usize, usize)>) {
    match link_type {
        LinkType::Item => {
            ...
            (db.status, db.entries.len(), db.error_msg.clone(), db.progress)
        }
        LinkType::Skill => {
...
pub fn search(link_type: LinkType, query: &str, filter_index: usize, max_results: usize) -> Vec<(u32, String)> {
    match link_type {
...
pub fn clear_all_caches() {
    ...
    delete_cache("db_items.json");
    delete_cache("db_skills.json");
    delete_cache("db_traits.json");
    delete_cache("db_recipes.json");
    delete_cache("db_skins.json");
    delete_cache("db_outfits.json");
    delete_cache("db_pois.json");
