...
const CACHE_FILE: &str = "db_api_skins.json";
...
fn fetch_new_from_api() {
    ...
    let result = rt.block_on(async {
        ...
        let batch =
            super::fetcher::batch_fetch::<Skin>(&client, "/skins", &new_ids, &|fetched, total| {
                let mut db = DB.lock();
...
fn fetch_from_api() {
    ...
    let result = rt.block_on(async {
        ...
        let batch =
            super::fetcher::batch_fetch::<Skin>(&client, "/skins", &ids, &|fetched, total| {
                let mut db = DB.lock();
                db.progress = Some((fetched, total));
            })
            .await;
...
pub fn search(query: &str, filter_index: usize, max_results: usize) -> Vec<(u32, String)> {
    ...
    let filter = SkinFilter::ALL
        .get(filter_index)
        .copied()
        .unwrap_or(SkinFilter::All);
    db.entries
        .iter()
